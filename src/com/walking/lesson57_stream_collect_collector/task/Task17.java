    ...
    public Map<String, Integer> calculate(List<Department> departments) {
        return departments.stream()
                .collect(Collectors.groupingBy(Department::getName,
                        Collectors.flatMapping(department->department.getEmployees().stream().map(Employee::getAge),
                                Collectors.collectingAndThen(Collectors.maxBy(Comparator.comparing(Integer::compareTo)),
                                        maxAge -> {
                                            return maxAge.orElse(0);
                                        }))));
    }
}
